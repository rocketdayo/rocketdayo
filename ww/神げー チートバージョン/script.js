...
function keyDownHandler(e) {
    if(e.key === 'Right' || e.key === 'ArrowRight') {
        ...
        debugMode = !debugMode;  // デバッグモードのトグル
    }
}
    ...
    if (bullets.length < maxBullets) {
        if (debugMode) {
            ...
            for (let angle = 0; angle < 360; angle += 45) {
                ...
                bullets.push({ x: playerX + playerWidth / 2 - 2.5, y: playerY, dx: Math.cos(rad), dy: -Math.sin(rad) });
            }
        } else {
            // 通常の弾を発射する
            bullets.push({ x: playerX + playerWidth / 2 - 2.5, y: playerY, dx: 0, dy: -1 });
        }
    }
}
...
setInterval(fireBullet, 500);  // 弾を自動的に発射する間隔を設定
draw();
